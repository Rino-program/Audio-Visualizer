# From the Developer
Thank you for using Audio Visualizer.
This is the F-Droid version, which focuses on local playback.

## About This App
This is a tool for enjoying music files with beautiful visuals.
This app is developed as open source, and the code is available on GitHub.
https://github.com/Rino-program/Audio-Visualizer

## Future Plans
- Further feature improvements
- Developing it into a full-fledged music player
- Addressing user feedback

## License & Request
This application is open-source software released under the MIT License. You are completely free to use, modify, and distribute it.

## A Small Request from the Developer (Optional)
If you use this app for your projects or creations, we would be absolutely thrilled if you could share it on X (formerly Twitter) with the hashtag #AudioVisualizerFDroid, or mention the creator @Rinoprogram!
Knowing how this app is being used motivates us immensely. (Of course, this is entirely optional, so feel free to use it without any notification!)

## Early Release & Beta Distribution
The early release and beta versions are publicly available on GitHub Pages.Please access them through the GitHub repository mentioned above.We are tracking bugs and issues via GitHub Issues. Feel free to submit your feedback!

## Important Notes
- Supported formats vary depending on the device model and Android version.

## Contact Us
Please submit your comments and requests via the Issues section of the GitHub repository.
https://github.com/Rino-program/Audio-Visualizer/issues

---
We hope this enriches your music experience.


Translated with DeepL.com (free version)